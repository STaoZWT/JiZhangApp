...
class editMemberPickerArguments {  //上一个页面传入的所有参数
...
class _editMemberPicker extends State<editMemberPicker>{
  ...
  Widget build(BuildContext context){
    final editMemberPickerArguments args = ModalRoute.of(context).settings.arguments;
    memberList = (isChange == false)?JsonDecoder().convert(args.legacyMemberPickerData): memberList;
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton( //添加新的account
        ...
        onPressed:() async {
          ...
          if(newMember != null){
            bool isExist = false;
            memberList.forEach((element) {  //检查新建账户是否和已有账户重复
            ...
            if(isExist == true) {
              ...
              setState(() {
                ...
                isChange = true;
                setPicker('mmemberPicker',JsonEncoder().convert(memberList)); //新list存入文件
              });
            }
          }
        },
